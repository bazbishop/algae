...
import algae.selector.TournamentSelector;
...
public class SelfReferencingSentence {
	...
	/**
	 * Off the shelf chromosome factory. If the phenotype mapper derives counter
	 * values by summing allele values from both chromosomes then the range of
	 * the allele values can be reduced.  
	 */
	...
	private static final IPhenotypeMapper phenotypeMapper = new IPhenotypeMapper() {
		...
		/**
		 * The phenotype consists of the derived values of the counters for each
		 * letter as well as the constructed sentence.
		 * Note that 10 of the letters are constant, since these letters do not
		 * occur in any of the numbers likely to occur in the finished sentence.
		 */
		@Override
		public Object createPhenotype(Genome genome) {
			...
			result.append("this sentence contains ");
			...
			for(int i = 0; i < 26; ++i) {
				...
				case 2: v = 3; break;
				case 3: v = 2; break;
				case 4: v = computeCounter(counts1, counts2, 0); break;
				case 5: v = computeCounter(counts1, counts2, 1); break;
				case 6: v = computeCounter(counts1, counts2, 2); break;
				case 7: v = computeCounter(counts1, counts2, 3); break;
				case 8: v = computeCounter(counts1, counts2, 4); break;
				case 9: v = 1; break;
				case 10: v = 1; break;
				case 11: v = computeCounter(counts1, counts2, 5); break;
				case 12: v = 1; break;
				case 13: v = computeCounter(counts1, counts2, 6); break;
				case 14: v = computeCounter(counts1, counts2, 7); break;
				case 15: v = 1; break;
				case 16: v = 1; break;
				case 17: v = computeCounter(counts1, counts2, 8); break;
				case 18: v = computeCounter(counts1, counts2, 9); break;
				case 19: v = computeCounter(counts1, counts2, 10); break;
				case 20: v = computeCounter(counts1, counts2, 11); break;
				case 21: v = computeCounter(counts1, counts2, 12); break;
				case 22: v = computeCounter(counts1, counts2, 13); break;
				case 23: v = computeCounter(counts1, counts2, 14); break;
				case 24: v = computeCounter(counts1, counts2, 15); break;
				case 25: v = 1; break;
				}
	...
	private static IFitnessTester fitnessTester = new IFitnessTester() {
		...
		/**
		 * Measure the error resulting from differences in the 'guessed' letter
		 * counts and the actualy count of letters in the complete sentence.
		 */
		@Override
		public IFitness fitness(Object phenotype) {
			...
			for(int i = 0; i < 26; ++i) {
				...
				//error += Math.abs(delta); //delta * delta;	// Least squares
				error += delta * delta;							// Linear sum
			}
	...
	public static void main(String[] args) {		
		...
		while (!finished) {
			...
			/*
			 * Experiment changing the mutation probability to avoid the population
			 * becoming homogeneous.
			 * This requires using the UniquePopulation container type.

			var population = lifeCycle.getCurrentPopulation();
			  
			if(1.0 * population.discarded() / population.size() > 0.005) {
				parameters.setElitismCount(0);
				//mutation *=.0;
				//mutation = Math.min(mutation, 0.2);
				mutation = 0.2;
			}
			else {// if(population.discarded() < 200 && mutation > 0.0001) {
				parameters.setElitismCount(0);
				mutation *= 0.5;
